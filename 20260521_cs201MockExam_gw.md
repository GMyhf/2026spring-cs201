...
Dijkstra, http://dsbpython.openjudge.cn/2026moni1re/010/
...
http://dsbpython.openjudge.cn/2026moni1re/013/
...
1. **重建二叉搜索树（BST）**：由于输入的是二叉搜索树的先序遍历序列，我们可以按照该序列的顺序依次将节点插入到树中，从而还原出原二叉树的结构。
2. **转换为累加树（GST）**：累加树的定义是每个节点的值等于原树中大于或等于该节点值的所有节点值之和。在二叉搜索树中，右子树的所有值都大于当前节点，左子树的所有值都小于当前节点。因此，我们可以通过**反向中序遍历**（右 -> 中 -> 左）的方式，从大到小遍历所有节点，并使用一个全局累加变量更新每个节点的值。
