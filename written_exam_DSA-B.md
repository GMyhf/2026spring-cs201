...
Updated 1102 GMT+8 Jun 22, 2026
...
（2）向一个有127个元素的顺序表中插入一个新元素并保持原来顺序不变，平均要移动的元素个数为（ B ）。
A．8   B．63.5    C．63   D．7
...
（5）根为第一层，深度为h的满m叉树的第k层有（ A ）个结点。（1≤k≤h）
...
> 这道题的经典标准答案通常是 **C．后序**（在很多标准考试和教材中，后序遍历被认为是最自然、最合适的解答，尽管前序和层次遍历在功能上也可以实现）。
>
> 以下是针对各个选项的详细分析和对比：
>
>  **1. 后序遍历（C．最合适）**
>
> * **原理**：采用自底向上的思想。先递归地将当前结点的左子树中的所有结点进行左右交换，再递归地将右子树中的所有结点进行左右交换，最后交换当前结点的左、右子树位置。
>* **优点**：这种“先子树、后根结点”的处理顺序非常符合分治法的逻辑，结构清晰，实现简单且不需要额外的辅助空间（除了递归栈）。因此，在学术和考试标准中，通常认为**后序遍历**最合适。
> 
>---
> 
> **2. 前序遍历（A．可行，但稍逊于后序）**
>
> * **原理**：采用自顶向下的思想。先交换当前结点的左、右子树，然后再递归地分别交换新左子树和新右子树。
>* **分析**：虽然前序遍历同样可以正确完成交换，但由于是先交换再递归，在直观逻辑上不如后序（先处理好子问题，再合并）更符合二叉树递归构建的常规思维。不过在实际编程中，前序和后序的效率和空间复杂度是相同的。
> 
>---
> 
>**3. 按层次遍历（D．可行，但需要辅助空间）**
> 
>* **原理**：利用队列，逐层访问结点并交换其左右孩子。
> * **分析**：这种方法在逻辑上可行，但因为需要引入一个外部队列来保存结点，空间复杂度会受到二叉树最大宽度的影响。相比之下，递归遍历利用的是系统栈，不需要手动维护辅助数据结构。
> 
>---
> 
>**4. 中序遍历（B．不可行/不合适）**
> 
>* **原理**：先递归遍历左子树，然后交换当前结点的左右子树，最后递归遍历右子树。
> * **分析**：如果按照标准的中序遍历步骤进行交换，当左子树遍历完并交换了当前结点的左右子树后，原来的右子树变成了左子树，而下一步要遍历的“右子树”实际上是已经遍历过的原左子树。这会导致原左子树被重复交换两次（恢复原样），而原右子树一次都没有被交换。因此，标准中序遍历无法正确完成该操作。
>
> **总结**
> 
> 在实际考试和理论考核中，该题的首选答案是 **C．后序**。
...
> 选 C。D 在某些实现或固定表长场景下可能间接受记录数影响，但教材通常强调哈希表平均查找长度主要取决于哈希函数、冲突处理方法和装填因子，而不能简单说“和记录总数有关”。
...
A. O(nlog n)   B. O(1)    C. O(n)          D. $O(n^2)$
...
A 采用邻接表存储，存储该图的空间复杂度为O(m+n)。
B 若为稠密图，更倾向于采用邻接矩阵存储。
C 采用邻接表存储，删除一个顶点的最坏情况的时间复杂度为O(n+m)。
...
4. （ Y ）通常不能通过一棵二叉树的前序遍历结点序列和后序遍历结点序列来确定这棵二叉树的完整结构。
...
> 在 Python 中，使用 **`is not`** 确实比使用 `!=` 更好、更符合 Python 的编程规范（Pythonic）。
>
> 以下是具体原因：
>
> **1. 语义更准确（标识对比 vs 值对比）**
>
> * **`is not`** 比较的是**对象的身份（Identity）**，即两个变量是否指向内存中的同一个对象（相当于 C/C++ 中的指针比较）。
> * **`!=`** 比较的是**对象的值（Value）**。由于我们没有在 `Node` 类中重写 `__eq__` 方法，默认情况下 `!=` 也会退化为比较内存地址，但在逻辑上，我们这里确实是在判断“这两个指针是否指向同一个结点”。因此，使用 `is not` 能够更精确地表达“是否为同一个结点”的语义。
>
> **2. 避免潜在的 bug**
>
> 如果后续由于某种需求，在 `Node` 类中重写了 `__eq__` 方法（例如，只要 `data` 相同就认为结点相等），那么使用 `!=` 就会导致遍历提前终止，从而产生逻辑错误。而使用 `is not` 比较的是内存地址，不会受到 `__eq__` 重写的影响，程序依然可以稳健地运行。
>
> ---
>
> **修改后的代码写法**
>
> 如果您在实际编写 Python 代码，以下写法是更推荐的：
>
> * **（3）** `ptr.next is not self.tail`
> * **（5）** `ptr is not self.tail.next`
>
> *(注：如果这是在参加某些有自动阅卷系统的考试或在线评测（OJ），有时系统的标准答案可能直接沿用了 C/C++ 的思维习惯，仅录入了 `!=`。在保证代码运行正确的前提下，如果是人工阅卷，使用 `is not` 通常会得到阅卷老师的认可。)*
...
def dfs(node, visited, graph, n):
    ...
    for i in range(n):
        if i in graph[node] and _____(1)_____:  # 1分
            count += __________(2)__________  # 1分
    return count
...
3. （ N ）对任意一个连通的无向图，如果存在一个环，且这个环中的一条边的权值不小于该环中任意一个其他的边的权值，那么这条边一定不会是该无向图的最小生成树中的边。
...
2. 定义完全二叉树的根结点所在层为第一层。如果一个完全二叉树的第六层有 23 个叶结点，则它的总结点数可能为 _ _ _ _ （请填写所有 3 个可能的结点数，写对 1 个得 1 分，2 个得 1.5 分，写错 1 个不得分）。**54,80,81**

3. 对于初始排序码序列（51, 41, 31, 21, 61, 71, 81, 11, 91），用双指针原地交换实现，第 1 趟快速排序（以第一个数字为中值）的结果是： _ _ _ _ 。**11 41 31 21 51 71 81 61 91**

4. 如果输入序列是已经正序，在（改进）冒泡排序、直接插入排序和直接选择排序算法中， _ _ _ _ 算法最慢结束。**直接选择排序**
...
>
> 一种最坏情况的示例如下：
> * **文本串 $S$**：由 $n$ 个字符 `'a'` 组成的字符串（即 $\underbrace{aa\dots a}_{n}$）。
> * **模式串 $P$**：由 $m-1$ 个字符 `'a'` 和 $1$ 个字符 `'b'` 组成的字符串（即 $\underbrace{aa\dots a}_{m-1}b$）。
>
> **原因说明**：在该情况下，对于 $S$ 中每个可能的匹配起点，算法在比对到 $P$ 的最后一个字符 `'b'` 之前，前 $m-1$ 个字符都会匹配成功。直到最后一个字符时才会发现不匹配（`'a'` 与 `'b'` 不匹配），从而不得不将模式串后移一位。这导致在每个位置都需要进行 $m$ 次字符比对，总比对次数为 $(n-m+1) \times m$，时间复杂度达到 $O((n-m+1)m)$。
>
> ---
>
> **b）朴素算法比对过程与结果**
>
> 已知 $S = \text{"abaabaabaabcc"}$（长度 $n=13$），$t = \text{"abaabc"}$（长度 $m=6$）。
>
> 采用朴素算法（Brute Force）逐个位置进行比对，具体步骤如下：
>
> 1. **第 1 趟（$i=0$）**：
>    * 比对 $S[0..5]$ 与 $t[0..5]$（"abaaba" 与 "abaabc"）。
>    * 前 5 个字符相同，第 6 个字符不匹配（$S[5]=\text{'a'} \neq t[5]=\text{'c'}$）。
>    * **比对次数：6 次**。
>
> 2. **第 2 趟（$i=1$）**：
>    * 比对 $S[1]$ 与 $t[0]$（$\text{'b'} \neq \text{'a'}$）。
...
>
> 3. **第 3 趟（$i=2$）**：
>    * 比对 $S[2..3]$ 与 $t[0..1]$（"aa" 与 "ab"）。
>    * 第 1 个字符匹配，第 2 个字符不匹配（$S[3]=\text{'a'} \neq t[1]=\text{'b'}$）。
>    * **比对次数：2 次**。
>
> 4. **第 4 趟（$i=3$）**：
>    * 比对 $S[3..8]$ 与 $t[0..5]$（"abaaba" 与 "abaabc"）。
>    * 前 5 个字符相同，第 6 个字符不匹配（$S[8]=\text{'a'} \neq t[5]=\text{'c'}$）。
>    * **比对次数：6 次**。
>
> 5. **第 5 趟（$i=4$）**：
>    * 比对 $S[4]$ 与 $t[0]$（$\text{'b'} \neq \text{'a'}$）。
...
>
> 6. **第 6 趟（$i=5$）**：
>    * 比对 $S[5..6]$ 与 $t[0..1]$（"aa" 与 "ab"）。
>    * 第 1 个字符匹配，第 2 个字符不匹配（$S[6]=\text{'a'} \neq t[1]=\text{'b'}$）。
>    * **比对次数：2 次**。
>
> 7. **第 7 趟（$i=6$）**：
>    * 比对 $S[6..11]$ 与 $t[0..5]$（"abaabc" 与 "abaabc"）。
>    * 所有字符均匹配成功。
>    * **比对次数：6 次**。
>
> * **总比对次数**：$6 + 1 + 2 + 6 + 1 + 2 + 6 = 24$ 次。
> * **查找结果**：匹配成功。模式串 $t$ 在文本串 $S$ 中首次出现的位置索引（0起算）为 **6**（即第 7 个字符处）。
>
> ---
>
> **c）避免冗余比对的参数调整**
>
> 在 $i=0, j=5$ 发生不匹配时，已匹配的子串为 $t[0..4] = \text{"abaab"}$。
>
> 为了避免冗余比对，可以利用已匹配部分的信息（类似于 KMP 算法的思想）：
> * 子串 $\text{"abaab"}$ 的最长相等前后缀为 $\text{"ab"}$，其长度为 2。
> * 这意味着模式串的前缀 $\text{"ab"}$ 可以直接对齐到文本串中已经匹配过的后缀 $\text{"ab"}$ 处（即 $S[3..4]$ 处）。
>
> 因此，下一次比对时：
> * 模式串在文本串中的起始对齐位置 $i$ 调整为 **3**。
> * 模式串中下一次待比对的字符索引 $j$ 调整为 **2**（即直接从模式串的第 3 个字符 `'a'` 开始与 $S[5]$ 进行比对，无需重新比对前两个字符）。
...
def topoSort(G):    # G 是邻接表，顶点从 0 开始编号
    ...
    q = queue.Queue()
    # q 是队列, q.put(x)可以将 x 加入队列，q.get()取走并返回队头元素
    # q.empty()返回队列是否为空
...
4. 为了实现一个循环队列（或称环形队列），采用数组 Q[0..m-1]作为存储结构，其中变量 rear 表示这个循环队列中队尾元素的实际位置，添加结点时按 rear=(rear+1) % m 进行指针移动，变量 length 表示当前队列中的元素个数，请问这个循环队列的队首元素的实际位置是（ B ）。
...
9. （ Y ）用邻接矩阵法存储一个图时，在不考虑压缩存储的情况下，所占用的存储空间大小只与图中结点个数有关，而与图的边数无关。
...
5. 已知二叉树的前序遍历结果 （先根周游序列） 为 ADC， 这棵二叉树的树型有 _ _ _ _ 种可能。**（5）**
   ...
   > 给定 3 个互不相同的结点，若只固定前序遍历序列为 ADC，则可能的二叉树树型数仍等于 3 个结点的二叉树形态数，即 Catalan 数 C3 = 5。
   >
   > 原因是：先确定任意一种二叉树形态，再按前序访问顺序依次把 A、D、C 填入结点，就能得到一棵前序遍历为 ADC 的二叉树；反过来，每棵前序为 ADC 的二叉树也对应唯一一种形态。因此固定前序序列并不会减少树型数量。
...
1）根据初始建堆算法，从最后一个非叶子节点开始，按数组下标递减的顺序逐个向下调整，保证每个子树满足小根堆性质。
